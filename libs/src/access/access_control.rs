...
pub const ACCESS_ROLE_MEMBER_KEY: &str = "_access_control_members";
pub const ACCESS_ROLE_ADMIN_KEY: &str = "_access_control_admin";
...
pub fn has_role(role: U256, account: Key) -> bool {
    get(ACCESS_ROLE_MEMBER_KEY, &get_role_members_key(role, account))
}
...
pub fn ret_has_role() {
    ...
    ret(has_role(role, account))
}

pub fn get_role_admin(role: U256) -> U256 {
    get(ACCESS_ROLE_ADMIN_KEY, &get_role_admin_key(role))
}

pub fn ret_role_admin() {
    let role: U256 = runtime::get_named_arg("role");

    ret(get_role_admin(role))
}
...
pub fn grant_role() {
    ...
    check_only_role(get_role_admin(role));
...
pub fn _set_role_admin(role: U256, admin_role: U256) {
    set(ACCESS_ROLE_ADMIN_KEY, &get_role_admin_key(role), admin_role);
}

pub fn _grant_role(role: U256, account: Key) {
    if !has_role(role, account) {
        set(
            ACCESS_ROLE_MEMBER_KEY,
            &get_role_members_key(role, account),
            true,
        );
    }
}
